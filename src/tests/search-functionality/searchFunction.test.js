import {
  ...
  test("test coordinateSearch", async () => {
    ...
    expect(lng.snippet).toBe("test test test {latitude: 12.3, longitude: -45.6} test test test")
  ...
  test("test urlSearch", async () => {
    ...
    const output = urlSearch(url_Search.rootUrl, url_Search.reqUrl, url_Search.classifications)
  ...
  test("test ipSearch", async () => {
    ...
    const output = ipSearch(ip_search.strReq, ip_search.ipObj, ip_search.rootUrl, ip_search.reqUrl)
    const ip = output[0]
    expect(ip.permission).toBe("tracking")
    expect(ip.snippet).toBe("6789 test test test 12345 test test test 108-26-182-44 test test test test 12345 test test test 6789 ")
