...
import {
  ...
  hashUserDictValues,
  createKeywordObj,
} from "../../background/analysis/buildUserData/structureUserData.js";
...
test("test general hashUserDictValues", async () => {
  ...
  expect(
    hashUserDictValues(general).personal.general[0].keywordHash
  ).toBeTruthy();
});

test("test createKeywordObj", async () => {
  expect(
    createKeywordObj("123\\D?45\\D?678\\D?910", "ipAdress").keywordHash
  ).toBeTruthy();
  expect(
    createKeywordObj("/testRegion/", "Region", "222222222").keywordHash
  ).toBe("222222222");
});
