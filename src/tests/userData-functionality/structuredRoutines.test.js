...
test("test getRegion", async () => {
  ...
  expect(getRegion("TestRegion")).toStrictEqual(/TestRegion/i);
});
...
test("test buildGeneralRegex", async () => {
  expect(buildGeneralRegex("myEmail@gmail.com")).toStrictEqual(
    /myEmail.?gmail.?com/
  );
  expect(buildGeneralRegex("(123)-456-7890")).toStrictEqual(
    /.?123.?.?456.?7890/
  );
