export const filterLabelObject = (allLabels, permFilter, companyFilter = null, runCompanyFilter = false) => {
                ...
                for (const [rootUrl, requests] of Object.entries(value)) {
                    for (const [url, evLevel] of Object.entries(requests)) {
                        for (const [typ, e] of Object.entries(evLevel)) {
                            ...
                            if (parent != null && parent in companyFilter && companyFilter[parent] ) {
                                //pass
                            }
                            else {
                                delete updatedLabels[perm][rootUrl][url][typ]
                                if (Object.keys(updatedLabels[perm][rootUrl][url]).length == 0){
