  ...
  function iterateMatchArray(deltaBound, typ) {
    ...
    while (i < matchArr.length) {
      ...
      if (deltaLat < deltaBound) {
        i = findPair(matchArr, absLng, i, startIndex, deltaBound, typ);
      } else if (deltaLng < deltaBound) {
        i = findPair(matchArr, absLat, i, startIndex, deltaBound, typ);
      } else {
        i += 1;
      }
    }
    ...
    if (idxKeyword != -1) {
      output.push(
        createEvidenceObj(
          ...
          strReq,
          reqUrl,
          typeEnum.fingerprinting,
          [idxKeyword, idxKeyword + keyword.length]
        )
      );
      break;
    }
    ...
    if (searchIndex != -1) {
      ...
      output.push(
        createEvidenceObj(
          ...
          strReq,
          reqUrl,
          typeEnum.trackingPixel,
          [reqUrlIndex, reqUrlIndex + reqUrl.length]
        )
      );
    }
  }
