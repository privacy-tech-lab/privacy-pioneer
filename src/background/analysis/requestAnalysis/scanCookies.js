...
import { regexSearch, coordinateSearch, locationKeywordSearch, ipSearch, encodedEmailSearch } from "./searchFunctions.js"
  ...
  function runWatchlistAnalysis(cookieString) {
    ...
    if ( typeEnum.phoneNumber in watchlistDict ) {
      watchlistDict[typeEnum.phoneNumber].forEach( number => {
        executeAndPush(regexSearch(cookieString, number, rootUrl, reqUrl, typeEnum.phoneNumber))
      })
    }
    if ( typeEnum.emailAddress in watchlistDict ) {
        executeAndPush(encodedEmailSearch(cookieString, networkKeywords, rootUrl, reqUrl))
       
        watchlistDict[typeEnum.emailAddress].forEach( email => {
        executeAndPush(regexSearch(cookieString, email, rootUrl, reqUrl, typeEnum.emailAddress))
      })
    }

    if ( typeEnum.userKeyword in watchlistDict ) {
      watchlistDict[typeEnum.userKeyword].forEach ( keyword => {
        executeAndPush(regexSearch(cookieString, keyword, rootUrl, reqUrl, typeEnum.userKeyword))
      })
    }
