...
if (isChrome) {
  ...
  async function changeFavicon () {
    await chrome.tabs.query({ active: true, currentWindow: true }, function (tabs){
      const currentWindowId= tabs[0].id
      ...
      const swapFavicon = async () => {
        ...
        if (numEvidence > EVIDENCE_THRESHOLD) {
          ...
          chrome.browserAction.setIcon({tabId: currentWindowId, path:"../assets/favicon2.svg"})
        } else {
          // Change it back to the original. Sometimes quickly changing a webpage after load
          // keeps the now-incorrect favicon
          chrome.browserAction.setIcon({tabId: currentWindowId, path:"../assets/favicon.svg"})
        }
      }
  ...
  chrome.webNavigation.onDOMContentLoaded.addListener(changeFavicon)
} else {
  /**
   * changes the favicon if above a certain threshold
   * 
...
chrome.downloads.onChanged.addListener(async function (downloadDelta) {
  ...
  if (status === "complete" || status === "interrupted") {
    ...
    const downloadItemArr = await chrome.downloads.search({ id: id })
